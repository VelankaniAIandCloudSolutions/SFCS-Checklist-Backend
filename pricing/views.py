...
from store_checklist.models import BillOfMaterials, BillOfMaterialsLineItem, Product
...
def get_product_pricing(request,product_id):
    ...
    vepl_parts = ['PCB','Allen Screws','Consultancy Charges']
    vepl_part_latest_prices  = []
    for purchase_order in purchase_orders[:5]:
        if len(vepl_parts)!=0:
            ...
            for vepl_part in vepl_parts:
                for po_line_item in po_line_items:
                    ...
                    print(vepl_part)
                    print(po_line_item.get('name'))
                    if vepl_part == po_line_item.get('name'):
                        print('name match ')
