        ...
        with transaction.atomic():
                ...
                if  str(row['VEPL Part No'])!='nan' and str(row['VEPL Part No']).strip().startswith('VEPL'):
                    ...
                    for mfr, mfr_part_no in zip(manufacturers, manufacturer_part_nos):
                        if mfr.strip() and mfr_part_no.strip():
                            ...
                            vepl_to_manufacturer_mapping[vepl_part_no].append(manufacturer_part)
                    ...
                    if 'Reference' in row and pd.notnull(row['Reference']):
                        for reference in row['Reference'].split(','):
                            ...
                            vepl_to_references_mapping[vepl_part_no].append(ref.name)
                    ...
                    existing_item = BillOfMaterialsLineItem.objects.filter(part_number=row['VEPL Part No'],bom=bom).first()
                    ...
                    if existing_item:
                        ...
                        if existing_item.part_number in vepl_to_manufacturer_mapping:
                            existing_item.manufacturer_parts.set(vepl_to_manufacturer_mapping[vepl_part_no])

                        if existing_item.part_number in vepl_to_references_mapping:
                            reference  = BillOfMaterialsLineItemReference.objects.filter(name = vepl_to_references_mapping[vepl_part_no]).first()
            ...
            for bom_line_item in bom_line_items:
                ...
                if vepl_part_no in vepl_to_manufacturer_mapping:
                    bom_line_item.manufacturer_parts.set(vepl_to_manufacturer_mapping[vepl_part_no])
                    bom_line_item.save()
